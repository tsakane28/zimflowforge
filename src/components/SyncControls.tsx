...
export function SyncControls() {
  ...
  const nextCheckAt = useFxStore((s) => s.nextCheckAt);
  const nextCheckReason = useFxStore((s) => s.nextCheckReason);
  ...
  const nextEta = nextCheckAt
    ? new Date(nextCheckAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
    : null;

  return (
    <div className="flex flex-col gap-2 bg-card border border-border rounded-lg px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col min-w-0">
          ...
          {nextEta && (
            <span className="text-[10px] font-mono text-muted-foreground/80 truncate">
              Next auto-check ~{nextEta}{nextCheckReason ? ` · ${nextCheckReason}` : ""}
            </span>
          )}
        </div>
      </div>
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-md bg-primary text-primary-foreground px-3 py-2 text-xs font-medium hover:bg-primary/90 disabled:opacity-60 self-start sm:self-auto"
      >
